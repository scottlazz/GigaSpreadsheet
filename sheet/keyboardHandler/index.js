...
export default class KeyboardHandler {
    ...
    async onKeyDown(e) {
        ...
        const formulaFocus = (_a = this.sheet.formulaBar) === null || _a === void 0 ? void 0 : _a.isEditing;
        const isEditingCellSelect = (_b = this.sheet.formulaBar) === null || _b === void 0 ? void 0 : _b.isEditingCellSelect;
        ...
        // Escape key to cancel editing
            ...
            if ((c === null || c === void 0 ? void 0 : c.row) == null)
                return;
            const value = ((_c = this.sheet.getCell(c.row, c.col)) === null || _c === void 0 ? void 0 : _c.bold) || false;
            (_d = this.sheet.toolbar) === null || _d === void 0 ? void 0 : _d.set('bold', value);
        }
            ...
            console.log({
                ...
                autosize: (_f = (_e = this.sheet.options) === null || _e === void 0 ? void 0 : _e.autosize) !== null && _f !== void 0 ? _f : false,
            ...
            if (!this.sheet.selectionEnd || isLocked)
                return;
            e.preventDefault();
            this.sheet.probe.style.display = 'block';
            this.handleArrowKeyDown(e);
        }
        else if (this.sheet.selectionStart && ((_g = e.key) === null || _g === void 0 ? void 0 : _g.length) === 1) {
            if (isLocked)
                return;
    ...
    handleArrowKeyDown(e) {
        if (!this.sheet.selectionEnd || !this.sheet.selectionStart)
            return;
        const deltas = {
        ...
        if (e.shiftKey) {
            ...
            if (e.key === 'ArrowUp' || (e.key === 'Enter')) {
                ...
                while (row > 0) {
                    ...
                    if (prevRect !== JSON.stringify(curRect))
                        break;
                    row--;
                }
            }
            ...
            if (e.key === 'ArrowUp') {
                row = merge.startRow - 1;
            }
            else if (e.key === 'ArrowDown' || e.key === 'Enter') {
                row = merge.endRow + 1;
            }
            else if (e.key === 'ArrowLeft') {
                col = merge.startCol - 1;
            }
        ...
        if (e.shiftKey && e.key !== 'Enter')
            this.sheet.selectionEnd = { row, col };
        ...
        if (this.sheet.selectedCell) {
            ...
            if (arrows.has(e.key) && e.shiftKey) {
                ...
                corner.forEach(side => {
                    if (side === 't') {
                        this.sheet.probe.style.top = `${-20 - this.sheet.headerRowHeight}px`;
                    }
                    else if (side === 'b') {
                        this.sheet.probe.style.bottom = '-20px';
                    }
                    else if (side === 'l') {
                        this.sheet.probe.style.left = `${-20 - this.sheet.rowNumberWidth}px`;
                    }
                    else if (side === 'r') {
                        this.sheet.probe.style.right = '-20px';
                    }
                });
                this.sheet.probe.style.display = 'block';
