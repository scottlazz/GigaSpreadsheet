export class BottomBar {
    ...
    addListeners() {
        this.menu.addEventListener('click', (e) => {
            if (e.target.hasAttribute('data-tabid')) {
                if (e.target.getAttribute('data-tabid') === String(this.active))
                    return;
    ...
    addTab(name, id) {
        ...
        if (tabContainer.lastElementChild) {
            tabContainer.lastElementChild.insertAdjacentHTML('afterend', `<li data-tabid="${id}" class="active">Sheet${id}</div>`);
        }
        else {
