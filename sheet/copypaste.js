...
export function parseXML(xml) {
    ...
    const markOccupied = (rowStart, colStart, rowspan, colspan) => {
        for (let rr = rowStart; rr < rowStart + rowspan; rr++) {
            if (!occupied[rr])
                occupied[rr] = [];
            for (let cc = colStart; cc < colStart + colspan; cc++)
                occupied[rr][cc] = true;
        }
    };
    for (let row of (tbody.rows)) {
        ...
        for (let col of row.children) {
            ...
            if (color && color !== 'rgb(0, 0, 0)')
                cell.color = rgbToHex(color);
            const bc = s.getPropertyValue('background-color');
            if (bc && bc !== 'rgba(0, 0, 0, 0)')
                cell.bc = s.getPropertyValue('background-color');
            if (s.getPropertyValue('text-align') && s.getPropertyValue('text-align') !== 'left')
                cell.ta = s.getPropertyValue('text-align');
            const fw = s.getPropertyValue('font-weight');
            if (fw && (fw === 'bold' || parseInt(fw) >= 500))
                cell.bold = true;
            if (s.getPropertyValue('font-style') === 'italic')
                cell.italic = true;
            if (s.getPropertyValue('text-decoration') === 'underline')
                cell.ul = true;
            const fontSize = s.getPropertyValue('font-size');
            if (fontSize && (fontSize !== '12px')) {
                const match = fontSize.match(/^\d+/);
                if (match)
                    cell.fontSize = parseInt(match[0]);
            }
...
export function toXML(cells, getMerge) {
