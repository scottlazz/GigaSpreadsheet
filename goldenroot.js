...
document.addEventListener("DOMContentLoaded", (event) => {
    ...
    const LayoutClass = GoldenLayout || GoldenLayout;
    const layout = new LayoutClass(goldenConfig, container);
    layout.registerComponent('giga', (containerComp, componentState) => {
        ...
        if (maybeEl && maybeEl.appendChild) {
            maybeEl.appendChild(mount);
        }
        else if (maybeEl && maybeEl.append) {
            maybeEl.append(mount);
        }
        else if (containerComp.appendChild) {
            containerComp.appendChild(mount);
        }
        new Sheet(mount, Object.assign({}, fininit, { toolbar: false, cellHeaders: false, formulaBar: false }));
